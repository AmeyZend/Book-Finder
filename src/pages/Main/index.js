...
function Main() {
  ...
  async function fetchBooks() {
    setLoading(true);
    await api.get(`?title=${query}`).then((response) => {
      if (!response.data.numFound) {
        setLoading(false);
        return Toast.fire({
          icon: 'error',
          title: 'No books found with this name',
        });
      }

      const books = response.data.docs.map((book) => {
        const thumbnailUrl = book.cover_i
      ...
      Toast.fire({
        icon: 'error',
        title: 'Error fetching books. Please try again.',
      });
    });
  }
